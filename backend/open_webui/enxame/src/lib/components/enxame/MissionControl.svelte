<script lang="ts">
  ...
  async function sendMission() {
    ...
    try {
      ...
      setTimeout(() => {
         if(isProcessing) { // Only simulate if we are still waiting
            ...
            currentMission = {
              ...
              answer: 'Modo de Simulação: O backend Enxame não respondeu em /api/v1/enxame/query. Verifique se o servidor Python está rodando e a rota foi registrada no main.py.',
              confidence: 0.0
            };
...
<style>
  ...
  .confidence { font-size: 0.8rem; color: var(--accent); margin-bottom: 0.5rem; font-weight: bold; }
  ...
  .stat-card small { color: var(--text-dim); display: block; margin-bottom: 4px; }
  ...
  .time { opacity: 0.5; margin-right: 8px; white-space: nowrap; }
