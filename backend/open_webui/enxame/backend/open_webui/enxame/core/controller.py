...
from ..models.schemas import Mission, MissionLog, MissionStatus, ResourceUsage, ValidationResult
...
class Controller:
    def __init__(
        self, guard: Guard, librarian: Librarian, scheduler: Scheduler, specialists: SpecialistService, judge: Judge
    ):
        self.guard = guard
    ...
    async def process_request(self, query: str, user_id: str = 'anonymous') -> Mission:
        ...
        try:
            ...
            if not validation.allowed:
                ...
                mission.logs.append(
                    MissionLog(
                        timestamp=datetime.utcnow(), agent_id='guard', action='blocked', details=str(validation.reasons)
                    )
                )
                return mission
            ...
            mission.logs.append(
                MissionLog(
                    timestamp=datetime.utcnow(), agent_id='guard', action='validated', details='Security check passed'
                )
            )
            ...
            mission.logs.append(
                MissionLog(
                    timestamp=datetime.utcnow(), agent_id='scheduler', action='scheduled', details=f'Node: {node_id}'
                )
            )
            ...
            mission.logs.append(
                MissionLog(timestamp=datetime.utcnow(), agent_id='controller', action='error', error=str(e))
            )
        ...
        mission.resource_usage = ResourceUsage(
            cpu_percent=25.0, ram_percent=50.0, ram_used_mb=1024.0, ram_total_mb=8192.0
        )
