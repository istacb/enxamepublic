#!/bin/bash
# ENXAME Bee - Debian/Ubuntu Uninstaller
...
step "Removendo pacote .deb..."
if [[ "$DRY_RUN" != true ]]; then
    sudo apt-get remove --purge -y enxame-bee
    sudo apt-get autoremove -y
fi

step "Removendo binarios manuais..."
if [[ "$DRY_RUN" != true ]]; then
    sudo rm -f /usr/bin/bee
    sudo rm -f /usr/bin/enxame-install-ollama
