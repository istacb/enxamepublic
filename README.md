...
## ⚠️ Política de Modelos de IA

**Importante:** A instalação do ENXAME **NÃO modifica modelos existentes** na máquina do usuário.

- ✅ Se você já tem modelos instalados, eles serão mantidos intactos
- ✅ Se não houver modelos, apenas modelos **≥ 1.5B parâmetros** serão instalados
- ✅ Modelos recomendados: `qwen2.5:1.5b`, `qwen2.5:3b`, `llama3.2:3b`, `mistral:7b`, `gemma2:2b`
- ❌ Modelos < 1.5B (ex: `llama3.2:1b`) não são instalados automaticamente

Esta política garante que:
1. Configurações pessoais sejam preservadas
2. Apenas modelos com qualidade mínima sejam usados em produção
3. O usuário tenha controle total sobre seus modelos
