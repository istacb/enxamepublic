#!/usr/bin/env python3
"""
BEE - Desinstalador Debian/Ubuntu
==================================
...
def print_header():
    print("=" * 50)
    print("  BEE - Desinstalador Debian/Ubuntu")
    print("=" * 50)
...
def uninstall_ollama_debian():
    """Remove o Ollama no Debian/Ubuntu."""
    print("\nRemovendo Ollama do Debian/Ubuntu...")

    # Remover pacote via apt
    ...
    if result.returncode == 0:
        print("✓ Ollama removido via apt")
    else:
        print("⚠ Ollama não encontrado via apt ou já removido")

    # Remover também dependências não usadas
    result2 = subprocess.run(
        ["sudo", "apt", "autoremove", "-y", "ollama"],
        capture_output=True,
        text=True,
    )
    if result2.returncode == 0:
        print("✓ Dependências removidas via apt autoremove")
...
def main():
    print_header()
    print("\nEste desinstalador removerá os arquivos do BEE instalado no Debian/Ubuntu.")
    print()
