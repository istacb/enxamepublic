...
SENSITIVE_PATTERNS = [
    (
        re.compile(r"(?i)(password|passwd|pwd|senha)\s*[:=]\s*[\"']?[\w@#$%^&*!]+[\"']?", re.IGNORECASE),
        r'\1=***REDACTED***',
    ),
    (
        re.compile(r"(?i)(secret|chave|token|api_key|apikey)\s*[:=]\s*[\"']?[\w\-_.]+[\"']?", re.IGNORECASE),
        r'\1=***REDACTED***',
    ),
        ...
        if record.args:
            try:
                if isinstance(record.args, dict):
                    record.args = {
                        k: sanitize_log_message(str(v)) if isinstance(v, str) else v for k, v in record.args.items()
                    }
