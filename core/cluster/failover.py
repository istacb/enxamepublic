...
def atualizar_heartbeat(conn: sqlite3.Connection, node_id: str):
    ...
    agora = time.time()
    conn.execute('UPDATE nos SET ultimo_heartbeat = ? WHERE node_id = ?', (agora, node_id))
    conn.commit()
...
def marcar_no_como_suspeito(conn: sqlite3.Connection, node_id: str):
    ...
    conn.execute("UPDATE nos SET estado = 'suspeito' WHERE node_id = ?", (node_id,))
    conn.commit()
...
def marcar_no_como_inativo(conn: sqlite3.Connection, node_id: str):
    ...
    conn.execute("UPDATE nos SET estado = 'inativo' WHERE node_id = ?", (node_id,))
    conn.commit()
...
def eleger_substituto(conn: sqlite3.Connection, papel_vacante: str, no_caido: str) -> NodeInfo | None:
...
def notificar_nos_sobre_failover(nos_ativos: list[NodeInfo], papel_vacante: str, no_caido: str, no_eleito: NodeInfo):
    ...
    for no in nos_ativos:
        try:
            httpx.post(f'http://{no.ip}:{no.porta}/api/v1/notificacao', json=payload, timeout=5)
        except Exception as e:
            log_failover(f'Falha ao notificar {no.node_id}: {e}')


def registrar_evento(
    conn: sqlite3.Connection, tipo: str, descricao: str, no_afetado: str | None = None, acao_tomada: str | None = None
):
...
def processar_failover(conn: sqlite3.Connection, no_inativo: NodeInfo):
    ...
    if no_inativo.papel.lower() in papeis_criticos:
        ...
        if substituto:
            ...
            conn.execute('UPDATE nos SET papel = ? WHERE node_id = ?', (no_inativo.papel, substituto.node_id))
            conn.commit()
...
class FailoverManager:
    ...
    def registrar_node(
        self, node_id: str, ip: str, porta: int, papel: str, score: float = 0, metadata: dict | None = None
    ):
            ...
            while self.running:
                try:
                    ...
                    for no in nos_inativos:
                        ...
                        cur = self.conn.execute('SELECT estado FROM nos WHERE node_id = ?', (no.node_id,))
    ...
    def _limpar_suspeitos_recuperados(self):
        ...
        self.conn.execute(
            "UPDATE nos SET estado = 'ativo' WHERE estado = 'suspeito' AND ultimo_heartbeat > ?", (limite,)
        )
        self.conn.commit()
